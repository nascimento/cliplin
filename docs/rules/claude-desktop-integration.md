...
- |
  ...
  - Changes to Claude rules (e.g. new file in .claude/rules/) or to instructions/claude.md content must stay consistent with docs/rules/ai-host-integration.md for the shared MCP command (same command/args as other hosts)
  - If adding Claude-specific templates (e.g. Skill manifest, new rule), document the path and purpose in this rules file
- |
  Cross-references:
  - Shared MCP command and multi-host checklist: docs/rules/ai-host-integration.md
  - MCP server instructions: docs/adrs/004-mcp-server-instructions.md, docs/rules/system-modules.md (server must expose instructions for all hosts)
